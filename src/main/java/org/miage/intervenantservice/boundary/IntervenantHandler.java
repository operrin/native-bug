...
public class IntervenantHandler {
    ...
    public Mono<ServerResponse> updateIntervenant(ServerRequest request) {
        return this.is.getIntervenant(request.pathVariable("id"))
                .flatMap(r -> Mono.zip (data -> {
                        return IntervenantAssembler.assemble((Intervenant) data[0], (Intervenant) data[1]);
                    }, this.is.getIntervenant(request.pathVariable("id")),request.bodyToMono(Intervenant.class))
                    .flatMap(is::createIntervenant)
